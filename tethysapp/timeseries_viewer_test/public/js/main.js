...
function add_series_to_chart(chart,res_id,number1,unit_off) {
    ...
    index = current_url.indexOf("timeseries-viewer-test");
    ...
    data_url = base_url + 'timeseries-viewer-test/chart_data/' + res_id + '/'+src+'/';
    $.ajax({
        url: data_url,
        success: function(json) {
         ...
         number  = number2;
            ...
            for(key in data1)
            {
                dataPoints.push({x: key, y: data1['y']});
            }
            ...
            var newSeries =
            {
                ...
                name: site_name + ': Number of data points '+count,
                dataPoints: data1
            };
            console.log(chart)
            ...
            chart.options.data[number].visible = true
            //chart.axisY.setTitle({title:"this is the y-axis"})
            chart.render();
...
$('#hello').on('click', function () {
    ...
    console.log(chart.options.data[0].visible)

    if (chart.options.data[0].visible == true){
        console.log("true")
